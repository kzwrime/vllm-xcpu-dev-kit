...
pd_create_role() {
    ...
    pd_write_role_preset "$port" "$rpc_port" "$coord_port" "$ready_port_base" "$role_dir" "$storage_dir" "$preset_file"
...
pd_validate_transfer() {
    ...
    if grep -R "External Cache Hit" "$PD_ROOT"/decode*/logs >/dev/null 2>&1; then

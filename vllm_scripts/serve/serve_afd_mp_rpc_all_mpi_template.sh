...
MOE_RANK_COUNT="${USER_VLLM_EP_SIZE:?USER_VLLM_EP_SIZE must be set for AFD}"

if ! [[ "$RANK" =~ ^[0-9]+$ && "$SIZE" =~ ^[0-9]+$ && \
        "$USER_VLLM_DATA_PARALLEL_SIZE" =~ ^[1-9][0-9]*$ && \
        "$USER_VLLM_MPC_SIZE" =~ ^[1-9][0-9]*$ && \
        "$MOE_RANK_COUNT" =~ ^[1-9][0-9]*$ ]]; then
    echo "Invalid AFD rank topology: rank=$RANK size=$SIZE F=$MOE_RANK_COUNT" >&2
    exit 1
fi
ATTENTION_RANK_COUNT=$((USER_VLLM_DATA_PARALLEL_SIZE * USER_VLLM_MPC_SIZE))
AFD_WORLD_SIZE=$((ATTENTION_RANK_COUNT + MOE_RANK_COUNT))
if [ "$SIZE" -ne "$AFD_WORLD_SIZE" ]; then
    echo "AFD world size mismatch: MPI size=$SIZE, expected A${ATTENTION_RANK_COUNT}+F${MOE_RANK_COUNT}=$AFD_WORLD_SIZE" >&2
    exit 1
fi
if [ "$RANK" -eq 0 ]; then
    echo "AFD topology A=0-$((ATTENTION_RANK_COUNT - 1)) F=${ATTENTION_RANK_COUNT}-$((AFD_WORLD_SIZE - 1))"
fi

if [ "$RANK" -ge "$ATTENTION_RANK_COUNT" ]; then
    MOE_RANK=$((RANK - ATTENTION_RANK_COUNT))
...
# Only Attention ranks participate in the vLLM RPC coordination service.
if [ "${VLLM_USE_MPI_COORD:-0}" == "1" ]; then
    ...
    echo "--- 🔗 MPI Coordination (Attention Rank $RANK) ---"
    ...
    if [ -f "$VLLM_MPI_ENV_EXPORT_FILE" ]; then
        ...
        echo "ERROR: Environment export file not found!" >&2
        exit 1
    fi
    echo ""
fi
...
if (( USER_VLLM_PP_SIZE * USER_VLLM_TP_SIZE != USER_VLLM_MPC_SIZE )); then
    echo "USER_VLLM_PP_SIZE($USER_VLLM_PP_SIZE) * USER_VLLM_TP_SIZE($USER_VLLM_TP_SIZE) != USER_VLLM_MPC_SIZE($USER_VLLM_MPC_SIZE)" >&2
    exit 1
fi
...
export VLLM_MP_RPC_READY_BASE_PORT=$((${VLLM_MP_RPC_READY_PORT_BASE:-28888} + DP_RANK * USER_VLLM_MPC_SIZE))

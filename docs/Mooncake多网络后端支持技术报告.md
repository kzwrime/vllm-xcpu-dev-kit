...
```bash
cd /shared/vllm-xcpu-dev-kit-0.19/deps/Mooncake
cmake -S . -B build-mooncake-customnic \
  -DUSE_TCP=ON -DUSE_CUSTOMNIC=ON
cmake --build build-mooncake-customnic --target transfer_engine_validator -j
```

当前仓库尚未内置 `USE_CUSTOMNIC` CMake option；在未实现新增后端前，上述配置会成功，
但 CMake 会提示 `USE_CUSTOMNIC` 未被使用。因此它只能验证基础 Transfer Engine 构建是否
可用，不能证明 `customnic` 已接入工厂。新增后端时需要先补齐 option、CMake target 和
`MultiTransport` 工厂分支，再用同一命令确认该 option 被实际消费。
...
需要 3 个终端，或者把 metadata server 和 target 放到后台运行。三个终端的作用和
预期执行时间如下：
...
这里使用 `scripts/http_metadata_server.py` 是为了让本地 smoke test 在没有额外 Python 包和
Go module 下载的情况下可复现。它只用 Python 标准库实现测试所需的
...
```bash
# 终端 2: target，常驻等待 initiator，不要提前 Ctrl-C。
...
如改用 `P2PHANDSHAKE`，initiator 的 `--segment_id` 不能直接填 target 的
`--local_server_name`，需要填 target 日志中
`RPC using P2P handshake, listening on ...` 打印的动态 RPC 地址。
...
./transfer_engine_validator --mode=target --protocol=rdma \
  --metadata_server=http://127.0.0.1:18080/metadata --local_server_name=<target_ip:port> \
...
./transfer_engine_validator --mode=initiator --protocol=rdma \
  --metadata_server=http://127.0.0.1:18080/metadata --local_server_name=<init_ip:port> \
...
旧版 `tcp_transport_test` 和 `transfer_metadata_test` 默认依赖 `MC_METADATA_SERVER`。若直接用空值会走 etcd 并 fatal；若强行用 `P2PHANDSHAKE`，测试内硬编码的 `127.0.0.2:12345` 会和 P2P 动态端口不一致。因此本次使用 `scripts/http_metadata_server.py` 这个内存型 HTTP metadata server，等价实现 `GET/PUT/DELETE /metadata?key=...`，并设置：

选择该脚本的原因是降低本地复现门槛：它只依赖 Python 标准库，不需要安装 `aiohttp`，
也不需要构建 Go 版 metadata server 或下载 Go modules。更正式的测试应使用 etcd 或
Mooncake 自带 metadata server。etcd 方式需要运行 etcd 服务端，并让所有测试进程使用同一个
`--metadata_server=<etcd_host>:2379`；Go 版 HTTP metadata server 需要 Go toolchain 和
`github.com/gin-gonic/gin` 等 module；Python 版 HTTP metadata server 需要 `aiohttp`。
...
ctest --test-dir build --output-on-failure \
  -R '^(default_config_test|transport_uint_test|endpoint_store_test|endpoint_store_integration_test|tcp_transport_test|transfer_metadata_test|topology_test|common_test)$'
```
...
该测试同样需要 3 个终端：终端 1 运行 HTTP metadata server 并常驻；终端 2 运行
target 并常驻等待；终端 3 运行 initiator，按 `--duration=3` 运行约 3 秒后自动退出。
target 和 metadata server 在 initiator 退出后再停止。

```bash
# 终端 1: HTTP metadata server，常驻到测试结束。
python3 scripts/http_metadata_server.py --host 127.0.0.1 --port 18080
```

```bash
# 终端 2: target，常驻等待 initiator。
...
Test completed: duration 3.00, batch count 2146, throughput 0.01 GB/s
```
...
benchmark smoke test 也需要 3 个终端：终端 1 运行 HTTP metadata server 并常驻；
终端 2 运行 benchmark target 并常驻等待；终端 3 运行 benchmark initiator，按
`--duration=3` 运行约 3 秒后自动退出。
...
```bash
# 终端 2: benchmark target，常驻等待 initiator。
...
```bash
# 终端 3: benchmark initiator，运行约 3 秒后自动退出。
./build/mooncake-transfer-engine/example/transfer_engine_bench \
...
```text
Worker 0 stopped!
Test completed: duration 3.00, batch count 4123, throughput 0.02 GB/s
```
